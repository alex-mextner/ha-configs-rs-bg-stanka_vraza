...
PY
then
    echo "[wakeword-recorder] completed session found at $SESSION_FILE; running pass-through capture without raw_live recording device=$DEVICE" >&2
    exec arecord -D "$DEVICE" -r "$RATE" -c "$CHANNELS" -f S16_LE -t raw \
